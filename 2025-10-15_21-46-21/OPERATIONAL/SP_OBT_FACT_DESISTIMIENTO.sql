CREATE PROCEDURE `amrl-data-prd`.OPERATIONAL.SP_OBT_FACT_DESISTIMIENTO()
OPTIONS(
  description="Propósito:Crear y actualizar la tabla OBT_FACT_DESISTIMIENTOS en el DataSet de COMSUPTION, La tabla muestra el detalle de los artículos incluidos en cada oportunidad DESISTIDA y permite filtros por subetapa y macroproyecto; consolida todos los atributos que se encuentran en la DIM_MACROPROYECTO, DIM_SUBETAPA, FACT_OPORTUNIDAD, DIM_INVENTARIO.\nAutor: Maria Fernanda Franco\nUsos: Tablero de Indicadores\nModificaciones: 2025-01-28: Se retira de la clausula WHERE la sentencia din.CLASE_DE_ARTICULO IN ( 'APT','CSA','LOC', 'OF')\n")
BEGIN
    CREATE OR REPLACE TABLE `amrl-data-prd.CONSUMPTION.OBT_FACT_DESISTIMIENTO` PARTITION BY FECHA_DE_SEPARACION CLUSTER BY SALA_DE_VENTAS, MACROPROYECTO ,MACROCONSOLIDADOR, TRANSACCIONAL AS
 ...
 SELECT  ROW_NUMBER() OVER (ORDER BY fo.BK_NUMERO_OP) AS SK_OBT_FACT_OPORTUNIDAD
       ...
       ,fo.SK_OP
        ...
        ,dpc.BK_COMPRADOR2 AS BK_COMPRADOR2
        ,UPPER(dpc2.NOMBRE_COMPLETO) AS COMPRADOR2
        ,dpc2.NUMERO_DE_IDENTIFICACION AS IDENTIFICACION_COMPRADOR2
        ...
        ,dpc2.TELEFONO AS TELEFONO_COMPRADOR2
        ,dpc2.GENERO AS GENERO_COMPRADOR2
        ,dpc2.ESTADO_CIVIL AS ESTADO_CIVIL_COMPRADOR2
...
LEFT JOIN `PRESENTATION.DIM_TIEMPO` dt ON DATE(fo.FECHA_DESISTIDO) = dt.FECHA
...
WHERE fo.DESISTIDO = 'Y'
      AND din.NOMBRE_ARTICULO IS NOT NULL;
     -- AND din.CLASE_DE_ARTICULO IN ( 'APT','CSA','LOC', 'OF'); se excluye 
     --where BK_COD_TRANSACCIONAL = 'CJMC23386'
    --AND BK_NUMERO_OP = '581518'
