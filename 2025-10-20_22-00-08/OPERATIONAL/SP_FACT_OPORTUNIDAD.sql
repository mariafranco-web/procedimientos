...
OPTIONS(
  description="Propósito: Crear y actualizar la tabla FACT_OPORTUNIDADES, consolidando todos los atributos que se encuentran en la pestaña resumen de la oportunidad de Oracle Fusion. La tabla muestra el total de oportunidades, la etapa en la que se encuentra la venta y el ID de los articulos asociados a la oporunidad (bk_articulo).\nAutor: Maria Fernanda Franco\nUsos: Tablero de Indicadores, TrackingTools\n2025-02-07: Se incluye la columna FECHA_SEPARACION_AJUSTADA\n2025-03-28: Se incluye CASE para la columna BK_PERSONA antes solo se llamaba el campo PRCONPARTYID\n2025-04-11: Se incluye LEFT JOIN con la tabla RAW.FUSION_ZCA_SALES_ORDER_HEADERS \n2025-05-14: Se incluye la condición ORDERTOTAL > 0 para la subtabla ZSOH\n2025-07-01: Se retira DISTINCT de la linea 6\n2025-07-29: Se incluye la subconsulta T1 para incluir los porcentajes de participación en la compra\n2025-10-03: Se incluye la subconsulta T2 para añadir campo Grupo de Asesores\n2025-10-14: Se modifica la subconsulta T2 para incluir los atributos relacionados al campo Grupo Responsable Asesor\n2025-10-16-MF: Se incluye subconsulta T3 y el campo ASESOR_RESPONSABLE_DEL_DESISTIMIENTO")
BEGIN
...
SELECT MO.OPTYID                                                                                AS SK_OP
                  ...
                  ,T3.PERSONNAMEPEODISPLAYNAME                                                  AS ASESOR_RESPONSABLE_DEL_DESISTIMIENTO 
            ...
            LEFT JOIN(-- T3: Obtiene el nombre del asesor que adjunta la carta de desisistimiento en la oportunidad (ASESOR_RESPONSABLE_DEL_DESISTIMIENTO)
                        SELECT *
                        FROM (SELECT CAST(A.PK1VALUE AS INT64) AS PK1VALUE
                                    ,ROW_NUMBER() OVER (PARTITION BY A.PK1VALUE ORDER BY CREATIONDATE1 DESC) AS row_num
                                    ,A.DOCUMENTID
                                    ,A.CREATIONDATE1
                                    ,A.CATEGORYNAME
                                    ,A.FILENAME
                                    ,A.DESCRIPTION1
                                    ,A.CREATEDBY1
                                    ,D.PERSONNAMEPEODISPLAYNAME 
                              FROM `RAW.FUSION_FND_ATTACHED_DOCUMENTS` A  
                              LEFT JOIN `RAW.FUSION_PER_USERS` C ON A.CREATEDBY1 = C.USERPEOUSERNAME
                              LEFT JOIN `RAW.FUSION_PER_PERSON_NAMES_F` D ON C.USERPEOPERSONID = D.PERSONNAMEPEOPERSONID  
                              WHERE  D.PERSONNAMEPEONAMETYPE = 'GLOBAL'  AND DESCRIPTION1 LIKE '%DESISTI%'
                              )
                        WHERE row_num = 1 --AND PK1VALUE = 300000072864319
            ) AS T3 ON T3.PK1VALUE = MO.OPTYID 
